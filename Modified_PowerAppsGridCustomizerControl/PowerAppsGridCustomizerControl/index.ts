...
export class GridCustomizerControl implements ComponentFramework.ReactControl<IInputs, IOutputs> {
    ...
    public init(
        ...
        context.utils.getEntityMetadata("contact", ["preferredcontactmethodcode"]).then((metadata)=>{
            const options = metadata.Attributes.get("preferredcontactmethodcode")?.attributeDescriptor.OptionSet as Array<any>|null|undefined;          
            ...
            if (eventName) {
                const paOneGridCustomizer: PAOneGridCustomizer = { 
                    cellRendererOverrides: generateCellRendererOverrides(colors, context), 
